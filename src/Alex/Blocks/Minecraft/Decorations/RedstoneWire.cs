	...
	public class RedstoneWire : RedstoneBase, IMultipartCheck
	{
		...
		public override IMaterial BlockMaterial
		{
			get
			{
				...
				return Material.Decoration.Clone().WithTintType(
					TintType.Color, Colors[Math.Clamp(power, 0, Colors.Length - 1)]);
			}
		}
		...
		public bool Passes(IBlockAccess world, Vector3 position, string rule, string value)
		{
			...
			if (value == "side|up")
			{
				return IsRedstoneWire(world, offset) || IsRedstoneWire(world, offset + Vector3.Down);
			}
			else if (value == "up")
			{
