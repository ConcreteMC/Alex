...
namespace Alex.Gamestates.Multiplayer
{
	public class MultiplayerAddEditServerState : GuiCallbackStateBase<MultiplayerAddEditServerState.AddOrEditCallback>
	{
		...
		public MultiplayerAddEditServerState(Action<AddOrEditCallback> callbackAction, GuiPanoramaSkyBox skyBox) : this(
			null, null, callbackAction, skyBox) { }

		private ServerTypeImplementation _selectedImplementation = null;
		private ServerTypeManager _serverTypeManager;

		public MultiplayerAddEditServerState(string name,
			string address,
			Action<AddOrEditCallback> callbackAction,
			GuiPanoramaSkyBox skyBox) : base(callbackAction)
		{
			...
			var hostnameRow = AddGuiRow(
				...
				{
					TabIndex = 2, Width = 200, PlaceHolder = "Hostname or IP", Margin = new Thickness(5),
				});

			hostnameRow.ChildAnchor = Alignment.MiddleCenter;
			hostnameRow.Orientation = Orientation.Horizontal;

			var typeLabelRow = AddGuiRow(
				_serverTypeLabel = new TextElement() { Text = "Server Type:", Margin = new Thickness(0, 0, 5, 0) });

			typeLabelRow.ChildAnchor = Alignment.MiddleCenter;
			typeLabelRow.Orientation = Orientation.Horizontal;

			AddGuiRow(
				_serverTypeGroup = new ButtonGroup()
				{
					Orientation = Orientation.Horizontal, ChildAnchor = Alignment.MiddleCenter
				});

			var buttonRow = AddGuiRow(
				_saveButton = new AlexButton(OnSaveButtonPressed)
				{
		...
		public MultiplayerAddEditServerState(SavedServerEntry entry,
			Action<AddOrEditCallback> callbackAction,
			GuiPanoramaSkyBox skyBox) : this(entry.Name, entry.Host + ":" + entry.Port, callbackAction, skyBox)
		{
			_entry = entry;
		}
			...
			foreach (var type in _serverTypeManager.GetAll())
			{
				...
				_serverTypeGroup.AddChild(
					element = new ToggleButton(type.DisplayName)
					{
						...
						DisplayFormat =
							new ValueFormatter<bool>((val) => $"{type.DisplayName} {(val ? "[Active]" : "")}"),
						TabIndex = tabIndex++
					}.ApplyModernStyle());
