	...
	public class OptionsStateBase : GuiMenuStateBase
	{
		...
		public OptionsStateBase(GuiPanoramaSkyBox skyBox)
		{
			...
			var footerRow = Footer.AddRow(
				BackButton = new GuiBackButton() { TranslationKey = "gui.done", Anchor = Alignment.TopFill, }
		...
		protected Button CreateLinkButton<TGameState>(string translationKey, string fallback = null)
			where TGameState : class, IGameState
		{
			...
			return new AlexButton(
				...
				}) { Text = fallback ?? translationKey, TranslationKey = translationKey, }.ApplyModernStyle(false);
		}

		protected Slider CreateSlider(Func<double, string> formatter,
			Func<AlexOptions, OptionsProperty<int>> optionsAccessor,
			int? minValue = null,
			int? maxValue = null,
			int? stepInterval = null) => CreateSlider(
			new ValueFormatter<double>(formatter), optionsAccessor, minValue, maxValue, stepInterval);
		...
		protected Slider CreateSlider(string label,
			...
			int? maxValue = null,
			int? stepInterval = null) => CreateSlider(
			new ValueFormatter<double>(label), optionsAccessor, minValue, maxValue, stepInterval);

		protected Slider CreateSlider(ValueFormatter<double> label,
			Func<AlexOptions, OptionsProperty<int>> optionsAccessor,
			int? minValue = null,
			int? maxValue = null,
			int? stepInterval = null)
		{
		...
		protected Slider CreateSlider(Func<double, string> formatter,
			Func<AlexOptions, OptionsProperty<double>> optionsAccessor,
			double? minValue = null,
			double? maxValue = null,
			double? stepInterval = null) => CreateSlider(
			new ValueFormatter<double>(formatter), optionsAccessor, minValue, maxValue, stepInterval);
		...
		protected Slider CreateSlider(ValueFormatter<double> valueFormatter,
			Func<AlexOptions, OptionsProperty<double>> optionsAccessor,
			double? minValue = null,
			double? maxValue = null,
		...
		protected Slider<TEnum> CreateSlider<TEnum>(Func<TEnum, string> formatter,
			Func<AlexOptions, OptionsProperty<TEnum>> optionsAccessor,
			TEnum minValue,
			TEnum maxValue) where TEnum : Enum => CreateSlider<TEnum>(
			new ValueFormatter<TEnum>(formatter), optionsAccessor, minValue, maxValue);
		...
		protected Slider<TEnum> CreateSlider<TEnum>(ValueFormatter<TEnum> valueFormatter,
			Func<AlexOptions, OptionsProperty<TEnum>> optionsAccessor,
			TEnum minValue,
			TEnum maxValue) where TEnum : Enum
		{
		...
		protected EnumSwitchButton<TEnum> CreateSwitch<TEnum>(string displayFormat,
			Func<AlexOptions, OptionsProperty<TEnum>> optionsAccessor) where TEnum : Enum
		{
			var @switch =
				CreateValuedControl<EnumSwitchButton<TEnum>, TEnum>(
					displayFormat, optionsAccessor); // {Modern = false, DisplayFormat = displayFormat};
		...
		protected TControl CreateValuedControl<TControl, TValue>(ValueFormatter<TValue> label,
			Func<AlexOptions, OptionsProperty<TValue>> propertyAccessor)
			where TControl : IGuiControl, IValuedControl<TValue>, new() where TValue : IConvertible
		{
		...
		protected TControl CreateValuedControl<TControl, TValue, TPropertyValue>(ValueFormatter<TValue> label,
			Func<AlexOptions, OptionsProperty<TPropertyValue>> propertyAccessor)
			...
			where TPropertyValue : IConvertible
		{
			...
			control.ValueChanged += (s, v) =>
				property.Value = (TPropertyValue)Convert.ChangeType(v, typeof(TPropertyValue));
