...
namespace Alex.Graphics.Models
{
	...
	public class SkyBox : IDisposable
	{
		...
		public SkyBox(IServiceProvider serviceProvider, GraphicsDevice device, World world)
		{
			...
			_moonPlaneVertices = new[]
			{
				...
				new VertexPositionTexture(
					new Vector3(planeDistance, 0, planeDistance), new Vector2(_moonX, _moonY)),
				new VertexPositionTexture(new Vector3(-planeDistance, 0, planeDistance), new Vector2(0, _moonY)),
			};

			MoonPlane = new VertexBuffer(
				device, VertexPositionTexture.VertexDeclaration, _moonPlaneVertices.Length, BufferUsage.WriteOnly);
		...
		private void SetupClouds(GraphicsDevice device, int planeDistance)
		{
			...
			CloudsPlane = new VertexBuffer(
				device, VertexPositionTexture.VertexDeclaration, cloudVertices.Length, BufferUsage.WriteOnly);
		...
		public float BrightnessModifier => MathHelper.Clamp(
			MathF.Cos(CelestialAngle * MathHelper.TwoPi) * 2 + 0.5f, 0.25f, 1f);
		...
		public Color WorldFogColor
		{
			get
			{
				...
				return new Color(
					0.7529412F * (f * 0.94F + 0.06F), 0.84705883F * (f * 0.94F + 0.06F), 1.0F * (f * 0.91F + 0.09F));
			}
		}
		...
		public Color AtmosphereColor
		{
			get
			{
				...
				var color = new Vector3(
					Blend(sky.X, fog.X, blendFactor), Blend(sky.Y, fog.Y, blendFactor),
					Blend(sky.Z, fog.Z, blendFactor));
		...
		public void Draw(IRenderArgs renderArgs)
		{
			...
			using (var context = GraphicsContext.CreateContext(
				       renderArgs.GraphicsDevice, BlendState.Opaque, DepthStencilState.DepthRead,
		...
		{
			SkyPlaneEffect.World = Matrix.CreateRotationX(MathHelper.Pi) * Matrix.CreateTranslation(0, 16, 0)
		...
		private void DrawClouds(IRenderArgs renderArgs, Vector3 position)
		{
			...
			CloudsPlaneEffect.World = Matrix.CreateTranslation(0, 16, 0) * Matrix.CreateTranslation(position);
		...
		{
			// Sun
			SunEffect.World = Matrix.CreateTranslation(0, 100, 0)
