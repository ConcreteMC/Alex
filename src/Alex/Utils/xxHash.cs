...
namespace Alex.Utils
{
	...
	public static class XXHash
	{
		...
		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		private static unsafe ulong processStripe64(ref byte* pInput,
			ref ulong acc1,
			ref ulong acc2,
			ref ulong acc3,
			ref ulong acc4,
			bool bigEndian)
		{
			...
			ulong acc = Bits.RotateLeft(acc1, 1) + Bits.RotateLeft(acc2, 7) + Bits.RotateLeft(acc3, 12)
		...
		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		private static unsafe (uint, uint, uint, uint) initAccumulators32(uint seed)
		{
			return (seed + prime32v1 + prime32v2, seed + prime32v2, seed, seed - prime32v1);
		}

		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		private static unsafe uint processStripe32(ref byte* pInput,
			ref uint acc1,
			ref uint acc2,
			ref uint acc3,
			ref uint acc4,
			bool bigEndian)
		{
			...
			return Bits.RotateLeft(acc1, 1) + Bits.RotateLeft(acc2, 7) + Bits.RotateLeft(acc3, 12)
			       + Bits.RotateLeft(acc4, 18);
		}
		...
		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		private static unsafe uint processRemaining32(byte* pInput, uint acc, int remainingLen, bool bigEndian)
		{
...
#pragma warning restore SA1401 // Fields should be private
		...
		{
			num = (Bits.RotateLeft(num, 48) & 0xFFFF0000FFFF0000ul) | (Bits.RotateLeft(num, 16) & 0x0000FFFF0000FFFFul);

			return (Bits.RotateLeft(num, 8) & 0xFF00FF00FF00FF00ul) | (Bits.RotateRight(num, 8) & 0x00FF00FF00FF00FFul);
		}
	}
}