...
namespace Alex.Gui.Elements.Map
{
	public class MapRenderElement : RocketControl
	{
		...
		public MapRenderElement(IMap map)
		{
			...
			AddChild(
				_north = new TextElement("North")
				{
					Anchor = Alignment.TopCenter, FontStyle = FontStyle.DropShadow, IsVisible = false
				});

			AddChild(
				_east = new TextElement("East")
				{
					Anchor = Alignment.MiddleRight, FontStyle = FontStyle.DropShadow, IsVisible = false
				});

			AddChild(
				_south = new TextElement("South")
				{
					Anchor = Alignment.BottomCenter, FontStyle = FontStyle.DropShadow, IsVisible = false
				});

			AddChild(
				_west = new TextElement("West")
				{
					Anchor = Alignment.MiddleLeft, FontStyle = FontStyle.DropShadow, IsVisible = false
				});
		}
		...
		private Vector2 GetRenderPosition(Vector3 position, Vector3 centerPosition, float scale)
		{
			position = Vector3.Transform(
				position,
				Matrix.CreateTranslation(-centerPosition) * Matrix.CreateRotationY(Rotation)
				                                          * Matrix.CreateTranslation(centerPosition));
	...
	public class MapClickedEventArgs : EventArgs
	{
		...
		public MapClickedEventArgs(ClickEventType eventType,
