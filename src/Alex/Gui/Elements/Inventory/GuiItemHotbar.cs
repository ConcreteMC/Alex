...
namespace Alex.Gui.Elements.Inventory
{
	public class GuiItemHotbar : Container
	{
		...
		public GuiItemHotbar(Utils.Inventories.Inventory inventory)
		{
			...
			{
				_hotbar.AddChild(
					_hotbarItems[i] = new GuiInventoryItem()
					{
						Width = ItemWidth + 4,
						Height = ItemWidth + 4,
		...
		protected override void OnDraw(GuiSpriteBatch graphics, GameTime gameTime)
		{
			...
			if (opacity > 0.1f)
			{
				var textSize = graphics.Font.MeasureString(_itemNameTextElement.Text, _itemNameTextElement.Scale)
				   .ToPoint();
				...
				var rect = new Rectangle(
					rectanglePosition.X - 2, rectanglePosition.Y - 2, textSize.X + 4, textSize.Y + 2);

				graphics.FillRectangle(rect, Color.Black * (0.75f) * opacity);

				graphics.DrawString(
					textPosition, _itemNameTextElement.Text, _itemNameTextElement.TextColor,
					_itemNameTextElement.FontStyle, _itemNameTextElement.Scale, opacity: opacity);
			}
		}
