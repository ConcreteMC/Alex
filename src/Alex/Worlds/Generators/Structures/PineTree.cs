...
namespace Alex.Worlds.Generators.Structures
{
	public class PineTree : TreeStructure
	{
		...
		protected void GenerateTopper(ChunkColumn chunk, Vector3 location, byte type = 0x0)
		{
			...
			chunk.SetBlock(x, y, z, BlockFactory.GetBlock(18, 1));
			//chunk.SetMetadata(x, y, z, 1);
			if (type == 0x1 && y < 256)
				GenerateVanillaCircle(chunk, new Vector3(x, y, z), sectionRadius, 18, 1);
		}

		public override void Create(World chunk, int x, int y, int z)
		{
		...
		protected void GenerateTopper(World chunk, Vector3 location, byte type = 0x0)
		{
			var sectionRadius = 1;
			GenerateCircle(chunk, location, sectionRadius, 18, 1);
			var top = location + new Vector3(0, 1, 0);
			...
			Block block = BlockFactory.GetBlock(18, 1);
