...
namespace Alex.Worlds.Generators.Structures
{
	...
	{
		protected void GenerateVanillaLeaves(World chunk, Vector3 location, int radius, byte id, byte meta)
		{
		...
		protected void GenerateVanillaCircle(World chunk, Vector3 location, int radius, byte id, byte meta,
	double corner = 0)
		{
			for (var I = -radius; I <= radius; I = (I + 1))
			{
				for (var j = -radius; j <= radius; j = (j + 1))
				{
					...
					if (max <= radius)
					{
						...
						var x = location.X + I;
						var z = location.Z + j;
						if (chunk.GetBlock(new BlockCoordinates((int)x, (int)location.Y, (int)z)).IsReplacible)
						{
							var block = BlockFactory.GetBlock(id, meta);
			...
			for (var I = -radius; I <= radius; I = (I + 1))
			{
				for (var j = -radius; j <= radius; j = (j + 1))
				{
					...
					if (max <= radius)
					{
						...
						if (chunk.GetBlock((int)x, (int)location.Y, (int)z).IsReplacible)
						{
							chunk.SetBlock((int)x, (int)location.Y, (int)z, BlockFactory.GetBlock(id, meta));
						}
					}
				}
			}
		...
		public static void GenerateColumn(ChunkColumn chunk, Vector3 location, int height, byte id, byte meta)
		{
			for (var o = 0; o < height; o++)
			{
				...
				chunk.SetBlock(x, y, z, BlockFactory.GetBlock(id, meta));
			}
		}

		public static void GenerateColumn(World chunk, Vector3 location, int height, byte id, byte meta)
		{
			for (var o = 0; o < height; o++)
			{
				var block = BlockFactory.GetBlock(id, meta);
			...
			for (var I = -radius; I <= radius; I = (I + 1))
			{
				for (var j = -radius; j <= radius; j = (j + 1))
				{
					...
					if (max <= radius)
					{
						...
						var z = (int)Z;
						if (chunk.GetBlock(x, y, z).IsReplacible)
						{
							chunk.SetBlock(x, y, z, BlockFactory.GetBlock(id, meta));
		...
		protected void GenerateCircle(World chunk, Vector3 location, int radius, byte id, byte meta)
		{
			for (var I = -radius; I <= radius; I = (I + 1))
			{
				for (var j = -radius; j <= radius; j = (j + 1))
				{
					...
					if (max <= radius)
					{
						...
						if (chunk.GetBlock(new BlockCoordinates(x,y,z)).IsReplacible)
						{
							Block block = BlockFactory.GetBlock(id, meta);
		...
		protected void GenerateBranch(ChunkColumn world, float x, float y, float z, double horDir, float verDir, float length, float speed, byte blockId, byte meta)
		{
			...
			while (c < length)
			{
				world.SetBlock((int)x, (int)y, (int)z, BlockFactory.GetBlock(blockId, meta));
		...
		protected void GenerateBranch(World world, float x, float y, float z, double horDir, float verDir, float length, float speed, byte blockId, byte meta)
		{
			if (verDir < 0f)
			{
				verDir = -verDir;
			...
			while (c < length)
			{
				Block block = BlockFactory.GetBlock(blockId, meta);
				...
				for (j = -s; j <= s; j++)
				{
					for (k = -w; k <= w; k++)
					{
						...
						{ 
							if (dist < 0.6f)
							{
								world.SetBlock(x + i, y + j, z + k, BlockFactory.GetBlock(woodId, woodMeta));
							//	world.SetMetadata(x + i, y + j, z + k, woodMeta);
							}
							if (world.GetBlock(x + i, y + j, z + k).IsReplacible)
							{
								world.SetBlock(x + i, y + j, z + k, BlockFactory.GetBlock(leafBlockId, leafMeta));
							}
						}
					}
				}
		...
		protected void GenerateLeaves(World world, int x, int y, int z, float size, float width, byte leafBlockId, byte leafMeta, byte woodId, byte woodMeta)
		{
			float dist;
			int i, j, k, s = (int)(size - 1f), w = (int)((size - 1f) * width);
			for (i = -w; i <= w; i++)
				...
				for (j = -s; j <= s; j++)
				{
					for (k = -w; k <= w; k++)
					{
						...
						if (dist <= size - 0.5f || (dist <= size && Rnd.NextDouble() < 0.5))
						{
							if (dist < 0.6f)
							{
								Block block = BlockFactory.GetBlock(woodId, woodMeta);
								block.Coordinates = new BlockCoordinates(x + i, y + j, z + k);

								world.SetBlock(block);
							}
							if (world.GetBlock(new BlockCoordinates(x + i, y + j, z + k)).IsReplacible)
							{
								Block block = BlockFactory.GetBlock(leafBlockId, leafMeta);
								block.Coordinates = new BlockCoordinates(x + i, y + j, z + k);
