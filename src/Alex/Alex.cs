...
namespace Alex
{
    public class Alex : Microsoft.Xna.Framework.Game
    {
        ...
        public VrService VrService { get; private set; }
        ...
        public Alex(LaunchSettings launchSettings)
        {
            ...
            serviceCollection.AddSingleton<Game>(this);
            serviceCollection.AddSingleton<GraphicsDevice>(sp => sp.GetRequiredService<Alex>().GraphicsDevice);
            serviceCollection.AddSingleton<GraphicsDeviceManager>(sp => sp.GetRequiredService<Alex>().DeviceManager);
            serviceCollection.AddSingleton<GuiManager>(sp => sp.GetRequiredService<Alex>().GuiManager);
            serviceCollection.AddSingleton<IGuiRenderer>(sp => sp.GetRequiredService<Alex>().GuiRenderer);
            serviceCollection.AddSingleton<GuiRenderer>(sp => sp.GetRequiredService<Alex>().GuiRenderer);
            serviceCollection.AddSingleton<GuiScaledResolution>(sp =>
                sp.GetRequiredService<Alex>().GuiManager.ScaledResolution);
            ...
            VrService = Services.GetService<VrService>();
        ...
        protected override void LoadContent()
        {
            ...
            GuiManager = new GuiManager(this, Services, InputManager, GuiRenderer, options, LaunchSettings.Vr);
            GuiManager.Init(GraphicsDevice, Services);

            if (VrService.Enabled)
            {
...
//                GuiManager.ScaledResolution.ViewportSize = new RocketUI.Size(width, height);
        ...
        {
            VrService.Draw(() => DoDraw(gameTime));
            ...
            // _spriteBatch.Draw(_leftEye,
            //     new Rectangle(0, 0, GraphicsDevice.Viewport.Width / 2, GraphicsDevice.Viewport.Height), Color.White);
            //
            // _spriteBatch.Draw(_rightEye,
            ...
            //     var fieldInfo = typeof(Texture2D).GetField("glTexture",
        ...
        private void ConfigureServices(IServiceCollection services)
        {
            ...
            services.TryAddSingleton<VrContext>(sp => VrContext.Get());
            services.TryAddSingleton<VrService>();
            ; //Storage = storage;
        }
