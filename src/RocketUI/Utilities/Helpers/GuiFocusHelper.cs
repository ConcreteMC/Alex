...
namespace RocketUI.Utilities
{
    public class GuiFocusHelper
    {
        ...
        private void UpdateInput()
        {
            ...
            if (CursorInputListener.IsBeginPress(GuiInputCommand.CursorClick))
            {
                FocusedElement = HighlightedElement;
            }

            if (HighlightedElement == FocusedElement && CursorInputListener.IsPressed(GuiInputCommand.CursorClick))
            {
                HighlightedElement?.InvokeCursorPressed(CursorPosition);
            }

            var isDown = CursorInputListener.IsDown(GuiInputCommand.CursorClick);
            if (isDown)
            {
        ...
        private bool TryFindNextControl(Vector2 scanVector, out IVisualElement nextControl)
        {
        ...
        public bool TryGetElementAt(Vector2 position, GuiElementPredicate predicate, out IVisualElement element)
        {
