...
namespace RocketUI.Utilities
{
	public class GuiDebugHelper
	{
		...
		private VisualElement TopMostHighlighted;
		private VisualElement TopMostFocused;
		...
		public void Update(GameTime gameTime)
		{
			...
			if (GuiManager.FocusManager.TryGetElementAt(CursorPosition, e => e is VisualElement c,
														out var controlMatchingPosition))
			{
				TopMostHighlighted = controlMatchingPosition as VisualElement;
			}
			else
			{
		...
		public void DrawScreen(IGuiScreen screen)
		{
			...
			if (HoverInfoEnabled)
			{
				...
				if (e != null)
				{
					var p = e.ParentElement as VisualElement;
		...
		private string GetElementInfo(VisualElement e)
		{
			var p = e.ParentElement as VisualElement;
		...
		private void DrawElementRecursive(IVisualElement element)
		{
			DrawDebug(element);
		...
		private void DrawDebug(IVisualElement element)
		{
		...
		private void DrawDebugBounds(Rectangle bounds, Color color, bool drawBackground = false, bool drawBorders = true, bool drawCoordinates = false, bool drawSize = false, bool highlight = false)
		{
			...
			if (drawCoordinates)
			{
				DrawDebugString(bounds.TopLeft(), $"({pos.X}, {pos.Y})", bgColor, fgColor, Anchor.BottomLeft);
			}
		...
		private void DrawDebugString(Vector2 position,   object obj, Anchor align = Anchor.TopLeft)
        {
            var x = (align & (Anchor.CenterX | Anchor.FillX)) != 0 ? 0 : ((align & Anchor.MinX) != 0 ? -1 : 1);
            var y = (align & (Anchor.CenterY | Anchor.FillY)) != 0 ? 0 : ((align & Anchor.MinY) != 0 ? -1 : 1);

            DrawDebugString(position, obj.ToString(), DebugTextBackground, DebugTextForeground, 2, x, y);
        }
        private void DrawDebugString(Vector2 position, object obj, Color color, Anchor align = Anchor.TopLeft)
        {
            var x = (align & (Anchor.CenterX | Anchor.FillX)) != 0 ? 0 : ((align & Anchor.MinX) != 0 ? -1 : 1);
            var y = (align & (Anchor.CenterY | Anchor.FillY)) != 0 ? 0 : ((align & Anchor.MinY) != 0 ? -1 : 1);
        ...
        private void DrawDebugString(Vector2 position, object obj, Color? background, Color color, Anchor align = Anchor.TopLeft)
        {
            var x = (align & (Anchor.CenterX | Anchor.FillX)) != 0 ? 0 : ((align & Anchor.MinX) != 0 ? -1 : 1);
            var y = (align & (Anchor.CenterY | Anchor.FillY)) != 0 ? 0 : ((align & Anchor.MinY) != 0 ? -1 : 1);
