...
namespace RocketUI.Input.Listeners
{
    public class MouseInputListener : InputListenerBase<MouseState, MouseButton>, ICursorInputListener
    {
        ...
        {
            RegisterMap(GuiInputCommand.CursorClick, MouseButton.Left);
        }
        ...
        {
            var delta = CurrentState.ScrollWheelValue - PreviousState.ScrollWheelValue;
            if (delta < 0) return -1;
            if (delta > 0) return 1;
            return 0;
        }
        ...
        {
            var delta = CurrentState.HorizontalScrollWheelValue - PreviousState.HorizontalScrollWheelValue;
            if (delta < 0) return -1;
            if (delta > 0) return 1;
            return 0;
        }
        ...
        {
            return new Vector2(GetXScrollDelta(), GetYScrollDelta());
        }
