...
namespace RocketUI
{
	public struct Thickness : IEquatable<Thickness>
	{
		...
		{
			if (string.IsNullOrWhiteSpace(thickness)) return Zero;
			...
			{
				// all
				return new Thickness(intParts[0]); 
			}
			else if (intParts.Length == 2)
			{
				return new Thickness(intParts[0], intParts[1]);
			}
			else if (intParts.Length == 4)
			{
				return new Thickness(intParts[0], intParts[1], intParts[2], intParts[3]);
			}
