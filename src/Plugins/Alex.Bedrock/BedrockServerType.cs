...
namespace Alex.Bedrock
{
	public class BedrockServerType : ServerTypeImplementation
	{
		...
		/// <inheritdoc />
		public BedrockServerType(BedrockPlugin plugin, XBLMSAService xblmsaService) : base(new BedrockServerQueryProvider(plugin), "Bedrock")
		{
			DefaultPort = 19132;
			Plugin = plugin;
			ProtocolVersion = McpeProtocolInfo.ProtocolVersion;
			XblmsaService = xblmsaService;
		}
		...
		{
			worldProvider = new BedrockWorldProvider(Plugin, Plugin.Game, connectionDetails.EndPoint,
		...
		{
			var profileManager = Plugin.Game.Services.GetService<ProfileManager>();
			var profileService = Plugin.Game.Services.GetService<IPlayerProfileService>();
			try
				{
					//Validate Bedrock account.
					//BedrockTokenPair tokenPair = JsonConvert.DeserializeObject<BedrockTokenPair>(profile.ClientToken);
					BedrockTokenPair tokenPair = JsonConvert.DeserializeObject<BedrockTokenPair>(profile.ClientToken);
					if (tokenPair.ExpiryTime < DateTime.UtcNow && await XblmsaService.TryAuthenticate(profile.AccessToken))
					{
					...
					return await XblmsaService.RefreshTokenAsync(tokenPair.RefreshToken).ContinueWith(task =>
						{
							if (task.IsFaulted)
							{
		...
		public override async Task<bool> VerifyAuthentication(PlayerProfile currentProfile)
		{
			...
			{
				var authenticationService = Plugin.Game.Services.GetService<IPlayerProfileService>();
		...
		public override async Task Authenticate(GuiPanoramaSkyBox skyBox, Action<bool> callBack)
		{
			...
			Plugin.Game.GameStateManager.SetActiveState(loginState, true);
		}
	}
}