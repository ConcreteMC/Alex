...
namespace Alex.Bedrock.Worlds
{
	public class BedrockClientPacketHandler : IMcpeClientMessageHandler
	{
        ...
        public BedrockClientPacketHandler(BedrockPlugin plugin, BedrockClient client, IEventDispatcher eventDispatcher, WorldProvider worldProvider, PlayerProfile profile, Alex alex, CancellationToken cancellationToken, ChunkProcessor chunkProcessor) //:
	       // base(client)
        {
	        Plugin = plugin;
		...
		public void HandleMcpeTransfer(McpeTransfer message)
		{
			 ...
			 ThreadPool.QueueUserWorkItem(o =>
			 {
				 ...
				 if (hostEntry.AddressList.Length > 0)
				 {
					 ...
					 AlexInstance.ConnectToServer(new BedrockServerType(Plugin, Plugin.AuthenticationService), new ServerConnectionDetails(new IPEndPoint(ip, message.port)), PlayerProfile);
				 }
			 });
		}
