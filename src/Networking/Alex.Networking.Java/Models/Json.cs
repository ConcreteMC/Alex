	...
	public static class Json
	{
		...
		private static JSONData String2Data(string toparse, ref int cursorpos)
		{
			try
			{
					...
					case '"':
						...
						while (toparse[cursorpos] != '"')
						{
							if (toparse[cursorpos] == '\\')
							{
								...
								{
									if (toparse[cursorpos + 1] == 'u' && isHex(toparse[cursorpos + 2])
				...
				while (cursorpos < toparse.Length && (char.IsWhiteSpace(toparse[cursorpos])
				                                      || toparse[cursorpos] == '\r' || toparse[cursorpos] == '\n'))
					cursorpos++;
