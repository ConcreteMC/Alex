...
namespace Alex.ResourcePackLib
{
		...
		IDisposable
	{
		...
		private ConcurrentDictionary<ResourceLocation, Func<Image<Rgba32>>> _bitmaps =
			new ConcurrentDictionary<ResourceLocation, Func<Image<Rgba32>>>();

		public IReadOnlyDictionary<ResourceLocation, Func<Image<Rgba32>>> Textures => _bitmaps;

		public IDictionary<ResourceLocation, EntityDescription> EntityDefinitions { get; private set; } =
			new ConcurrentDictionary<ResourceLocation, EntityDescription>();

		public IDictionary<string, AttachableDefinition> Attachables { get; private set; } =
			new ConcurrentDictionary<string, AttachableDefinition>();

		public IDictionary<string, RenderController> RenderControllers { get; private set; } =
			new ConcurrentDictionary<string, RenderController>();

		public IDictionary<string, AnimationController> AnimationControllers { get; private set; } =
			new ConcurrentDictionary<string, AnimationController>();
		...
		private static readonly Regex IsEntityModel = new Regex(
			@"^models[\\\/]entity[\\\/](?'filename'.*)\.json$", RegexOpts);

		private static readonly Regex IsRenderController = new Regex(
			@"^render_controllers[\\\/](?'filename'.*)\.json$", RegexOpts);

		private static readonly Regex IsAnimationController = new Regex(
			@"^animation_controllers[\\\/](?'filename'.*)\.json$", RegexOpts);
		...
		private static readonly Regex IsSoundDefinition = new Regex(
			@"^sounds[\\\/]sound_definitions\.json$", RegexOpts);
		...
		private static readonly Regex IsAttachableFile = new Regex(
			@"^attachables[\\\/](?'filename'.*)\.json$", RegexOpts);

		private static readonly Regex IsUiTexture = new Regex(
			@"^textures[\\\/]ui[\\\/](?'filename'.*)\.png", RegexOpts);
		...
		private void Load(ResourcePack.LoadProgress progressReporter)
		{
			...
			Dictionary<string, RenderController> renderControllers =
				new Dictionary<string, RenderController>(StringComparer.Ordinal);

			Dictionary<string, AnimationController> animationControllers =
				new Dictionary<string, AnimationController>(StringComparer.Ordinal);

			Dictionary<string, ParticleDefinition> particleDefinitions =
				new Dictionary<string, ParticleDefinition>(StringComparer.Ordinal);
		...
		private void ProcessAttachable(IFile entry, Dictionary<string, AttachableDefinition> attachableDefinitions)
		{
			try
			{
				...
				var versionedResource = MCJsonConvert.DeserializeObject<VersionedResource<AttachableDefinition>>(
					json,
					new VersionedResourceConverter<AttachableDefinition>(
						"minecraft:attachable", true, (def) => def.Description.Identifier));
		...
		private void ProcessParticle(IFile entry, Dictionary<string, ParticleDefinition> particleDefinitions)
		{
			try
			{
				...
				var versionedResource = MCJsonConvert.DeserializeObject<VersionedResource<ParticleDefinition>>(
					json,
					new VersionedResourceConverter<ParticleDefinition>(
						"particle_effect", true, (def) => def.Description.Identifier));
				...
				foreach (var controller in versionedResource.Values)
				{
					if (particleDefinitions.TryAdd(controller.Key, controller.Value))
					{
						...
						if (texture != null
						    && !_bitmaps.ContainsKey(controller.Value.Description.BasicRenderParameters.Texture))
						{
							TryAddBitmap(texture);
						}
					}
		...
		public static Dictionary<string, EntityModel> ProcessEntityModels(Dictionary<string, EntityModel> models,
			Func<string, EntityModel> lookup = null)
		{
			Dictionary<string, EntityModel> final =
				new Dictionary<string, EntityModel>(StringComparer.InvariantCultureIgnoreCase);
		...
		private void LoadEntityDefinition(IFile entry, Dictionary<string, EntityDescription> entityDefinitions)
		{
			...
			foreach (var e in obj)
			{
				...
				if (e.Key == "minecraft:client_entity" && e.Value != null)
				{
					...
					if (!clientEntity.TryGetValue(
						    "description", StringComparison.InvariantCultureIgnoreCase, out var descriptionToken)
					    || descriptionToken.Type != JTokenType.Object)
						continue;
					...
					if (!string.IsNullOrWhiteSpace(engineText))
					{
						desc.MinEngine = FormatVersionHelpers.FromString(engineText);
						         ...
						         "min_engine_version", StringComparison.InvariantCultureIgnoreCase,
						         out var minEngineVersion))
					{
		...
		private bool TryAddBitmap(string path)
		{
			...
			{
				return _bitmaps.TryAdd(path, new Func<Image<Rgba32>>(() => { return TryLoad(path + ".tga"); }));
			}
			else if (_archive.GetEntry(path + ".png") != null)
			{
				return _bitmaps.TryAdd(path, new Func<Image<Rgba32>>(() => { return TryLoad(path + ".png"); }));
			}
