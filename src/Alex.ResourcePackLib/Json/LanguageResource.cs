...
namespace Alex.ResourcePackLib.Json
{
	...
	{
		private static readonly Regex LangFileRegex = new Regex(
			@"^\s*(?'key'[\w\.]+)\s*=\s*(?'value'.+)\s*$",
			RegexOptions.Compiled | RegexOptions.Multiline | RegexOptions.IgnoreCase);

		[JsonProperty("language.name")] public string CultureName { get; set; }

		[JsonProperty("language.region")] public string CultureRegion { get; set; }

		[JsonProperty("language.code")] public string CultureCode { get; set; }

		public LanguageResource() { }

		public LanguageResource(IEnumerable<KeyValuePair<string, string>> values) : base(
