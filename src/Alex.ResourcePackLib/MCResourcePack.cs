...
namespace Alex.ResourcePackLib
{
	...
	{
		public delegate void McResourcePackPreloadCallback(IFontRenderer fontRenderer, BitmapFont bitmapFont);
		...
		private void Preload(ZipArchive archive)
		{
			...
			IsPreLoaded = true;
			PreloadCallback?.Invoke(AsciiFont, Font);
		}
				...
				if (fontTextureMatch.Success)
				{
					...
					if (textureMatch.Success)
					{
						if (fontTextureMatch.Groups["filename"].Value == "ascii")
						{
							...
							LoadTexture(entry, textureMatch);
						}
					}
				}
