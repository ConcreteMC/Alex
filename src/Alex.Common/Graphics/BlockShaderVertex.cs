...
namespace Alex.Common.Graphics
{
	...
	{
		public static readonly MinifiedBlockShaderVertex Default = new MinifiedBlockShaderVertex(
			Vector3.Zero, BlockFace.None, Vector4.Zero, Color.White);
		...
		public MinifiedBlockShaderVertex(Vector3 position, BlockFace normal, Vector4 texCoords) : this(
			position, normal, texCoords, Color.White, 0, 15)
		{
			//BlockLight = 0;
			//SkyLight = 15;
		}
		...
		public MinifiedBlockShaderVertex(Vector3 position, BlockFace normal, Vector4 texCoords, Color color) : this(
			position, normal, texCoords, color, 0, 15) { }
		...
		public MinifiedBlockShaderVertex(Vector3 position,
			BlockFace normal,
			Vector4 texCoords,
			Color color,
			byte blockLight,
		...
		public static VertexDeclaration VertexDeclaration { get; } = new VertexDeclaration(
			...
			new VertexElement(
				(8 * sizeof(float)) + (4 * sizeof(byte)), VertexElementFormat.Short2,
	...
	{
		public static readonly BlockShaderVertex Default = new BlockShaderVertex(
			Vector3.Zero, Vector3.Zero, Vector2.Zero, Color.White);
		...
		/// <param name="texCoords">The UV co-ords for this vertex</param>
		public BlockShaderVertex(Vector3 position, Vector3 normal, Vector2 texCoords) : this(
			position, normal, texCoords, Microsoft.Xna.Framework.Color.White) { }
		...
		/// <param name="texCoords">The UV co-ords for this vertex</param>
		/// <param name="color">The color of this vertex</param>
		public BlockShaderVertex(Vector3 position, Vector3 normal, Vector2 texCoords, Color color) : this(
			position, normal, texCoords, color, 0, 15) { }
		...
		/// <param name="texCoords">The UV co-ords for this vertex</param>
		/// <param name="color">The color of this vertex</param>
		public BlockShaderVertex(Vector3 position,
			Vector3 normal,
			Vector2 texCoords,
			Color color,
			byte blockLight,
		...
		public static VertexDeclaration VertexDeclaration { get; } = new VertexDeclaration(
			new VertexElement(0, VertexElementFormat.Vector3, VertexElementUsage.Position, 0),
			new VertexElement(
				(3 * sizeof(float)), VertexElementFormat.Vector2, VertexElementUsage.TextureCoordinate, 0),
			new VertexElement((5 * sizeof(float)), VertexElementFormat.Color, VertexElementUsage.Color, 0),
			new VertexElement(
				(5 * sizeof(float)) + (4 * sizeof(byte)), VertexElementFormat.Short2,
				VertexElementUsage.TextureCoordinate, 1),
			new VertexElement(
				(5 * sizeof(float)) + (4 * sizeof(byte)) + (2 * sizeof(short)), VertexElementFormat.Single,
