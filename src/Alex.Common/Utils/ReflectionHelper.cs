...
namespace Alex.Common.Utils
{
	public static class ReflectionHelper
	{
		...
		{
			var foundMethod = type.GetMethod(method, BindingFlags.NonPublic | BindingFlags.Instance);
		...
		public static T GetPrivateStaticPropertyValue<T>(Type t, string propName)
		{
			...
			if (pi == null)
				throw new ArgumentOutOfRangeException(
					"propName", string.Format("Property {0} was not found in Type {1}", propName));
		...
		public static T GetPrivateFieldValue<T>(Type t, object obj, string propName)
		{
			...
			if (fi == null)
				throw new ArgumentOutOfRangeException(
					"propName", string.Format("Field {0} was not found in Type {1}", propName, obj.GetType().FullName));
		...
		public static FieldInfo GetPrivateFieldAccesor(Type t, string propName)
		{
			...
			if (fi == null)
				throw new ArgumentOutOfRangeException(
					"propName", string.Format("Field {0} was not found in Type {1}", propName, t.FullName));
		...
		public static void SetPrivateStaticPropertyValue<T>(Type t, string propName, T val)
		{
			...
			if (t.GetProperty(propName, BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Static) == null)
				throw new ArgumentOutOfRangeException(
					"propName", string.Format("Property {0} was not found in Type {1}", propName, t.FullName));

			t.InvokeMember(
				propName, BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.SetProperty | BindingFlags.Static,
				null, null, new object[] { val });
		}
		...
		public static void SetPrivateStaticFieldValue<T>(Type t, string propName, T val)
		{
			...
			t.InvokeMember(
				propName, BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.SetField | BindingFlags.Static,
				null, null, new object[] { val });
		}
